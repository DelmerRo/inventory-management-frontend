...
// Utilidades puras fuera del componente
...
const Dashboard: React.FC = () => {
    ...
    // Cálculos analíticos adicionales basados en los datos del backend
    const analytics = useMemo(() => {
        if (!data) return null;
        
        // 1. Participación porcentual de los insumos sobre el capital total inmovilizado
        const packagingShare = data.capital.totalCombinedArs > 0 
            ? (data.capital.suppliesValueArs / data.capital.totalCombinedArs) * 100 
            : 0;

        // 2. Relación porcentual del costo de empaque frente al costo base del producto
        const packagingCostRatio = data.profitability.averageProductCost > 0
            ? (data.profitability.averagePackagingCost / data.profitability.averageProductCost) * 100
            : 0;

        // 3. Simulador de escenarios de margen rápido (Ej: -10%, Actual, +15%, +30%)
        const baseCostTotal = data.profitability.averageProductCost + data.profitability.averagePackagingCost;
        const simulatedMargins = [
            { label: 'Conservador (25%)', margin: 25, revenue: baseCostTotal * 1.25, profit: baseCostTotal * 0.25 },
            { label: 'Esperado (' + sliderValue + '%)', margin: sliderValue, revenue: data.profitability.potentialRevenue, profit: data.profitability.potentialNetProfit },
            { label: 'Optimista (50%)', margin: 50, revenue: baseCostTotal * 1.50, profit: baseCostTotal * 0.50 },
            { label: 'Premium (75%)', margin: 75, revenue: baseCostTotal * 1.75, profit: baseCostTotal * 0.75 },
        ];

        return { packagingShare, packagingCostRatio, simulatedMargins };
    }, [data, sliderValue]);
    ...
    return (
        <div className="max-w-7xl mx-auto p-4 md:p-8 pb-24 space-y-8">
            ...
            <div className="bg-gradient-to-r from-gray-900 to-gray-800 text-white rounded-3xl p-6 md:p-8 shadow-xl flex flex-col lg:flex-row justify-between items-start lg:items-center gap-8 relative overflow-hidden">
                ...
                <div className="relative z-10">
                    <div className="flex items-center gap-3 mb-2">
                        <h1 className="text-3xl md:text-4xl font-black tracking-tight">📊 Panel Financiero & Operativo</h1>
                        <button 
                            onClick={handleManualRefresh}
                            disabled={isLoading}
                    ...
                    <p className="text-gray-400 font-medium">Control unificado de inventario, costos ponderados (CPP) y proyecciones de rentabilidad</p>
                </div>
                ...
                <div className="w-full lg:w-96 bg-gray-900/50 backdrop-blur-sm p-5 rounded-2xl border border-gray-700 relative z-10 shadow-inner">
                    <div className="flex justify-between items-center mb-3">
                        <label className="text-sm font-bold text-gray-300 uppercase tracking-wider">Margen Objetivo</label>
                    ...
                    />
                    <p className="text-[10px] text-gray-400 mt-2 text-right">Deslice para recalcular ingresos y ganancias</p>
                </div>
            </div>
            ...
            {isLoading && !data ? (
                ...
                <>
                    {/* SECCIÓN 1: Tarjetas Principales (Capital, Rentabilidad, Predicciones) */}
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        <div className="bg-white rounded-3xl p-6 border border-gray-100 shadow-sm hover:shadow-md transition-shadow flex flex-col">
                            ...
                            <div className="mt-6 bg-gray-50 p-5 rounded-2xl border border-gray-100 group">
                                <div className="flex justify-between items-start mb-1">
                                    <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest group-hover:text-indigo-500 transition-colors">Total Consolidado</p>
                                    <span className="text-[10px] font-black bg-indigo-100 text-indigo-700 px-2 py-0.5 rounded">TC: ${data.capital.currentExchangeRate}</span>
                                </div>
                                ...
                                <p className="text-sm font-bold text-indigo-400 mt-1">
                                    {formatUSD(data.capital.totalCombinedUsd)} <span className="text-gray-400 text-xs font-medium">(Dólar oficial/referencia)</span>
                                </p>
                            </div>
                        </div>
                        ...
                        <div className="bg-white rounded-3xl p-6 border border-gray-100 shadow-sm hover:shadow-md transition-shadow flex flex-col">
                            ...
                            <div className="flex-1 space-y-4">
                                <div className="flex justify-between items-end border-b border-gray-50 pb-3">
                                    <span className="text-sm font-bold text-gray-500">Costo Base Promedio (CPP)</span>
                                    <span className="text-lg font-black text-gray-800">{formatARS(data.profitability.averageProductCost)}</span>
                                </div>
                                <div className="flex justify-between items-end border-b border-gray-50 pb-3">
                                    <span className="text-sm font-bold text-gray-500">Costo Empaque Promedio</span>
                    ...
                    {/* SECCIÓN 2: Reportes Avanzados y Análisis Estructural */}
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        
                        {/* Reporte A: Estructura de Costos y Participación de Insumos */}
                        <div className="bg-white rounded-3xl p-6 md:p-8 border border-gray-100 shadow-sm">
                            <h3 className="text-lg font-black text-gray-800 mb-6 flex items-center gap-2">
                                <span className="text-xl">📊</span> Eficiencia de Empaque y Costos
                            </h3>
                            
                            <div className="space-y-6">
                                <div>
                                    <div className="flex justify-between items-center text-xs font-bold text-gray-600 mb-2">
                                        <span>Peso del Embalaje sobre el Costo Base</span>
                                        <span className="text-indigo-600 font-black">{analytics.packagingCostRatio.toFixed(1)}%</span>
                                    </div>
                                    <div className="w-full bg-gray-100 h-3 rounded-full overflow-hidden">
                                        <div 
                                            className="bg-indigo-600 h-full rounded-full transition-all duration-500" 
                                            style={{ width: `${Math.min(analytics.packagingCostRatio, 100)}%` }}
                                        ></div>
                                    </div>
                                    <p className="text-[11px] text-gray-400 mt-1">Indica cuánto inciden las cajas, etiquetas y cintas en relación al costo directo del producto de decoración.</p>
                                </div>

                                <div>
                                    <div className="flex justify-between items-center text-xs font-bold text-gray-600 mb-2">
                                        <span>Insumos vs Mercadería (Capital Total)</span>
                                        <span className="text-blue-600 font-black">{analytics.packagingShare.toFixed(1)}% Insumos</span>
                                    </div>
                                    <div className="w-full bg-gray-100 h-3 rounded-full overflow-hidden flex">
                                        <div 
                                            className="bg-blue-500 h-full transition-all duration-500" 
                                            style={{ width: `${Math.min(analytics.packagingShare, 100)}%` }}
                                            title="Insumos"
                                        ></div>
                                        <div 
                                    ...
                                    <div className="flex justify-between text-[11px] text-gray-400 mt-1 font-medium">
                                        <span>🔵 Insumos: {formatARS(data.capital.suppliesValueArs)}</span>
                                        <span>🟢 Mercadería: {formatARS(data.capital.productsValueArs)}</span>
                                    </div>
                                </div>
                            </div>
                        </div>

                        {/* Reporte B: Simulador de Sensibilidad de Margen */}
                        <div className="bg-white rounded-3xl p-6 md:p-8 border border-gray-100 shadow-sm flex flex-col justify-between">
                            <div>
                                <h3 className="text-lg font-black text-gray-800 mb-4 flex items-center gap-2">
                                    <span className="text-xl">🧮</span> Simulador de Escenarios de Margen
                                </h3>
                                <p className="text-xs text-gray-400 font-medium mb-4">Proyección de ingresos y ganancias netas evaluando distintos objetivos de rentabilidad comercial.</p>
                            </div>
                                ...
                                <table className="w-full text-left border-collapse">
                                    <thead>
                                        <tr className="border-b border-gray-100 text-[10px] font-extrabold text-gray-400 uppercase tracking-wider">
                                            ...
                                            <th className="py-2 text-right">Ingreso Potencial</th>
                                            <th className="py-2 text-right">Ganancia Neta</th>
                                        </tr>
                                    </thead>
                                    <tbody className="divide-y divide-gray-50 text-xs">
                                        {analytics.simulatedMargins.map((item, idx) => (
                                            <tr key={idx} className={`hover:bg-gray-50/50 ${item.margin === sliderValue ? 'bg-indigo-50/40 font-bold' : ''}`}>
                                                <td className="py-3 font-bold text-gray-800 flex items-center gap-1.5">
                                                    {item.margin === sliderValue && <span className="w-1.5 h-1.5 rounded-full bg-indigo-600"></span>}
                                                    {item.label}
                                                </td>
                                                <td className="py-3 text-right font-black text-gray-900">{formatARS(item.revenue)}</td>
                                                <td className="py-3 text-right font-black text-emerald-600">+{formatARS(item.profit)}</td>
                                            </tr>
                                        ))}
                                    </tbody>
                                </table>
